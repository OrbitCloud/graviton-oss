create or replace package az_change_notifications is
  ...
  -- Purpose : Halda utan um CQ tilkynningar á töflubreytingum

  type t_clob is table of clob;

  procedure log_action(p_title in varchar2, p_message in clob);
  ...
  procedure enqueue(p_queue_id in number, p_table in varchar2, p_azure_queue in varchar2, p_data in clob);

  function get_rowdata(p_owner in varchar2, p_table in varchar2, p_rowid in varchar2) return clob;
  function append_row(p_table in varchar2, p_owner in varchar2, p_rowid in varchar2) return clob;
  function changed_rows(p_table in varchar2, p_owner in varchar2, p_dags in date) return sys.chnf$_rdesc_array;

  procedure enqueue_rowids(p_rowids in sys.chnf$_rdesc_array, p_regid in number, p_table in oqf_tables%rowtype);
  ...
  function generate_schema(p_table in varchar2) return clob;
