...
def az_storageaccount(
    ...
    if private_endpoints is not None:
        for endpoint_dict in private_endpoints:
            # This assumes that endpoint_dict is a dictionary with string keys.
            try:
                endpoint_config = PrivateEndpointConfig(**endpoint_dict)
            except TypeError as e:
                print(f"Error converting endpoint_dict to PrivateEndpointConfig: {e}")
                print(f"The problematic endpoint_dict: {endpoint_dict}")
                continue  # Skip this iteration and continue with the next one

            az_private_endpoint(
                resource=storageaccount,
                resource_group=resource_group,
                private_endpoint_config=endpoint_config,
                tags=tags,
            )
